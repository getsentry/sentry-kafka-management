...
def test_update_config_state_keep_emergency_config(
    ...
    # No changes needed since dynamic value already matches emergency config
    assert len(success) == 0
    assert len(errors) == 0

    # apply_configs should not be called since dynamic value already matches
    mock_apply_configs.assert_not_called()
...
@patch("sentry_kafka_management.actions.local.manage_configs.apply_configs")
@patch("sentry_kafka_management.actions.local.manage_configs.get_active_broker_configs")
def test_update_config_state_apply_emergency_config_when_dynamic_differs(
    mock_get_configs: MagicMock,
    mock_apply_configs: MagicMock,
    mock_remove_configs: MagicMock,
    mock_admin_client: MagicMock,
    ...
    """Test that emergency configs are applied when dynamic value differs."""
    (temp_record_dir / "num.network.threads").write_text("1000")

    temp_properties_file.write_text("broker.id=1001\n" "num.network.threads=500\n")
    ...
    mock_get_configs.return_value = [
        ...
        Config(
            ...
            is_sensitive=False,
            active_value="500",
            dynamic_value="500",  # Dynamic value differs from emergency config (1000)
            dynamic_default_value=None,
            static_value="500",
            default_value="3",
        ),
    ...
    # apply_configs should be called since dynamic value differs from emergency config
    mock_apply_configs.assert_called_once_with(
        mock_admin_client,
        {"num.network.threads": "1000"},
        ["1001"],
